...
function Movie_card(response, count, movies_list) {
  for (let i = 0; i < count; i++) {
    ...
    if (response.results[i].backdrop_path) {
      ...
      movie_image.addEventListener("click", (event) => {
        event.preventDefault();
        Single_movie(response, i);
      });
      ...
      // adding event listener click to movie_info as it is above image which prevent listening click on image
      movie_info.addEventListener("click", (event) => {
        event.preventDefault();
        Single_movie(response, i);
      });
...
function clickHandler(event) {
  ...
  after_movie_list.classList.remove("disappear");
...
function Single_movie(response, i) {
  ...
  movie_image.src =
    "https://image.tmdb.org/t/p/original" + response.results[i].poster_path;
  movie_image.alt = "poster";
